...
export async function rss2data(url, max = 5, order = "newFirst") {
  ...
  if (order == "random") {
    posts.sort(function (a, b) {
      return (
        new Date((isNotAtom ? b.pubDate : b.published ?? b.updated)["#text"]) -
        new Date((isNotAtom ? a.pubDate : a.published ?? a.updated)["#text"])
      );
    });
    newestFirst = true;
  }
  let entries = [];
  for (
    let entry = newestFirst ? 0 : posts.length - 1;
    newestFirst ? entry < max : entry >= posts.length - max;
    newestFirst ? entry++ : entry--
  ) {
    const entryDate = new Date(
      (isNotAtom ? posts[entry].pubDate : posts[entry].published ?? posts[entry].updated)["#text"]
    );
    const dateOptions = {
      weekday: "long",
      year: "numeric",
    ...
    entries.push({
      title:
        posts[entry].title["#text"] != ""
          ? posts[entry].title["#text"]
          : entryDate.toLocaleDateString("en-GB", dateOptions),
      link: entryLink,
